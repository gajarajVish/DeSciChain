...
class DatabaseService {
     ...
     */
    async getModelsWithMetadata(filters) {
        let query = `
      SELECT 
        pm.id,
     ...
     */
    async deletePendingModel(modelId) {
     ...
     */
    async getModelWithMetadata(modelId) {
        const query = `
      SELECT 
        pm.*,
        mm.name,
     ...
     */
    async createPendingPurchase(purchase) {
        const query = `
     ...
     */
    async getPendingPurchase(purchaseId) {
     ...
     */
    async confirmPurchase(purchaseId, updates) {
        const client = await this.getClient();
        try {
            await client.query('BEGIN');
            // Get pending purchase
            const pendingResult = await client.query('SELECT * FROM pending_purchases WHERE id = $1', [purchaseId]);
     ...
     */
    async getPurchase(modelId, buyerAddress) {
        const query = `
     ...
     */
    async createModelAccessKey(accessKey) {
        const query = `
     ...
     */
    async getModelAccessKey(modelId, buyerAddress) {
        const query = `
     ...
     */
    async incrementDownloadCount(accessKeyId) {
        const query = `
